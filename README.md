...
<p align="center">
  <img src="docs/architecture.svg" alt="Isometric view of TAAR's four planes: trigger, execution, state, serving" width="880">
</p>

Generated by [scripts/gen-architecture.mjs](scripts/gen-architecture.mjs) from
the design tokens, so it can be rebuilt when the architecture moves. Depth is
face shading rather than a drop shadow — the design system bans those.

The same thing in text, if you would rather read it:
